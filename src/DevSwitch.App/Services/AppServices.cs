...
internal sealed class AppSelfUpdateService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        //       本次更新开始前先清空整个 updates 目录（含所有历史版本残留），与 App 启动时的
        //       DataRootMaintenance.PurgeUpdatesStagingDirectory 形成双层保险：
        //       启动时清"上次重启前的残留"，本次清"启动后由其它来源遗留的产物"。
        //       不在「本次结束时」清理：因为外部 updater 在主程序退出后才异步从 extracted 复制，
        //       本次结束就删会删掉 updater 仍要读取的源文件。
        string updatesRoot = Path.Combine(dataRoot, "updates");
        DataRootMaintenance.PurgeDirectoryContents(updatesRoot);
