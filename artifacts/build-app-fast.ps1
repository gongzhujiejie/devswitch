...
$out = 'artifacts\app-build-fast.out.txt'
$err = 'artifacts\app-build-fast.err.txt'
Remove-Item $out, $err -ErrorAction SilentlyContinue
$p = Start-Process -FilePath 'I:\SoftWare\dotnet\dotnet.exe' -ArgumentList @('build', 'src\DevSwitch.App\DevSwitch.App.csproj', '--configuration', 'Debug', '--verbosity', 'minimal', '--no-restore') -WorkingDirectory 'I:\lpppp\envoy' -NoNewWindow -Wait -PassThru -RedirectStandardOutput $out -RedirectStandardError $err
